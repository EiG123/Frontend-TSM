...
const handleFileChange = (e: Event) => {
  ...
  if (target.files && target.files.length > 0) {
    files.value = [...files.value, ...Array.from(target.files)]; // merge ไฟล์เก่า+ใหม่
    result.value = null;
    error.value = null;
...
const uploadFile = async () => {
  ...
  try {
    ...
    const res = await networkAVAManage.UploadIncidentTT(formData); // ส่ง formData แทน file เดี่ยว
      ...
      <div class="space-y-6">
        ...
        >
          ...
          <div class="p-6 space-y-4">
            ...
            <!-- Selected File Info -->
            <!-- Selected Files — แทนที่ v-if="file" เดิม -->
